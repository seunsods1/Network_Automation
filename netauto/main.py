...
#int_lst_th[i],dom_pull_per_id[int_lst_th[i]],optest_per_sn, user_dict
...
#        optest_dict['powerbudget_dev_rx_percent'] = str(round(((optest_dict['powerbudget_dev_rx']/(endpoint_tx - atten))*100),2)) + ' %'
    ...
    #############Dictionary to hold optical data per interface ID############################
#    optest_per_int[port_id] = optest_dict

    ###########Dictionary to hold optical data per Serial number of optic################################################
    #optest_dict['port_id'] = port_id
    #optest_per_sn[optest_dict['optical_SN']] = optest_dict
...
def enter_so():
    ...
    """
    sw_vendor_ans_path_dict = json.dumps(sw_vendor_ans_path_dict)
    with open('sw_vendor_ans_path_dict.json', 'w') as fl:
        fl.write(sw_vendor_ans_path_dict)
    """
    ################################################################################################################
    ...
    """
    env = Environment(loader=FileSystemLoader('.'))
    templ = env.get_template("./ansiblefiles/usr_auth/usr_auth_end1/usr_auth_end1.j2")

    with open("./ansiblefiles/junos_ansiblefil/group_vars/all.yml", 'w') as fl:
        fl.write(templ.render(auth_dict=auth_dict))
    """
...
@app.route('/enter_new_so_new_swconfig', methods=["GET", "POST"])
def enter_new_so_new_swconfig():
    os.remove('./tmp_opdata.json')
    ...
    #        try:
        for k,v in testbed_viol_dict.items():
            ...
            if k == 'idprom_viol_dict':
                if v['err_cnt'] != 0:
                    idprom_err_loc,idprom_err_cnt,idprom_data = v['err_loc'],v['err_cnt'],v['data'][0]
            if k == 'inv_viol_dict':
                if v['err_cnt'] != 0:
