...
#    id_sn_dict = [line[line.index('SN:')+3:] for line in inv_lst2[ind[0]:ind[-1]] if 'SN:' in line]
    i = 0
    for line in testbed_inv2[sn_range[0]:sn_range[-1]]:
        if 'SN:' in line:
    ...
    for i in range(len(testbed_idprom)):
        if testbed_viol_dict['idprom_viol_dict'][i]['err_cnt'] > 0:
            #######removing character - 'V<i^X^Q.\\P.' from 'testbed_dom' because it leads to excessive spacing#####################
            for j in range(len(testbed_idprom[i])):
                if testbed_idprom[i][j] == 'V<i^X^Q.\\P.':
                    testbed_idprom[i][j] = ''
    ...
    slash_cnt = ideal_op_id.count('/')
    ideal_inv_lst = []
    start_ind = [ideal_op_id.index(i) for i in ideal_op_id if i.isdigit()][0]
    ideal_op_num = ideal_op_id[start_ind:]

    for i in range(len(ideal_inv)):
        if ideal_op_num in ideal_inv[i]:
            ideal_inv_line = [re.sub(' +',' ',ideal_inv[i])][0]+','+[re.sub(' +',' ',ideal_inv[i+1])][0]
            ideal_inv_lst = ideal_inv_line.split(',')
            ideal_inv_lst.pop(-1)
            ideal_inv_lst.pop(0)
            break
    ideal_inv_lst = [re.sub(' +','',i) for i in ideal_inv_lst]
    ...
    for i in range(len(testbed_inv)):
        if id_prefix in testbed_inv[i].lower() and testbed_inv[i].count('/') == slash_cnt:
            ...
            temp_tb_lst_1.pop(0)
            temp_tb_lst_1 = [re.sub(' +','',i) for i in temp_tb_lst_1]
            print('This is temp_tb_lst_1',temp_tb_lst_1)
            if temp_tb_lst_1 != ideal_inv_lst:
                v = temp_tb_lst_1 == ideal_inv_lst
                print(temp_tb_lst_1,',',ideal_inv_lst,',',v)
                testbed_viol_dict['inv_viol_dict']['err_loc'].append(i)
                testbed_viol_dict['inv_viol_dict']['err_loc'].append(i+1)
